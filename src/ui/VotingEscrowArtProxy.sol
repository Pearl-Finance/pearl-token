...
contract VotingEscrowArtProxy {
    using Strings for uint256;
    using StringUtils for uint256;

    /**
     * @notice Generates the ERC721 token URI for a given token ID in the voting escrow system.
     * @dev This function constructs the token metadata as a JSON string, encodes it in Base64, and then prepends the
     * appropriate data URI scheme. The metadata includes the token's name, description, and an SVG image (also Base64
     * encoded) generated by `_generateSVG`. The SVG image visually represents the token's locked amount and remaining
     * vesting duration.
     * @param tokenId The ID of the ERC721 token for which the URI is generated.
     * @param lockedAmount The amount of tokens locked in this escrow position, used in the SVG generation.
     * @param remainingVestingDuration The remaining duration of the token vesting, used in the SVG generation.
     * @return A string representing the complete ERC721 token URI.
     */
    function tokenURI(uint256 tokenId, uint256 lockedAmount, uint256 remainingVestingDuration)
        external
        pure
        returns (string memory)
    {
        string memory svg = _generateSVG(tokenId, lockedAmount, remainingVestingDuration);
    ...
    /**
     * @dev Generates an SVG image for a given token in the voting escrow system. This internal function creates an SVG
     * representation displaying the locked amount and remaining vesting duration of a token. The SVG is styled and
     * structured to visually represent the token's characteristics in the context of the voting escrow system.
     * @param tokenId The ID of the ERC721 token for which the SVG is generated.
     * @param lockedAmount The amount of tokens locked in this escrow position, displayed in the SVG.
     * @param remainingVestingDuration The remaining duration of the token vesting, displayed in the SVG.
     * @return A string representing the SVG image.
     */
    function _generateSVG(uint256 tokenId, uint256 lockedAmount, uint256 remainingVestingDuration)
        internal
        pure
        returns (string memory)
    {
        return string(
            abi.encodePacked(
                ...
                lockedAmount.formatUintToString(18),
                '</text></g><g class="st5"><text class="st2 st3 st6" transform="translate(20 263)">Remaining vesting duration</text></g><g class="st5"><text class="st2 st3 st6" transform="translate(280 263)" text-anchor="end">Token ID</text></g><g class="st1"><text class="st2 st3 st7" transform="translate(20 280)">',
                remainingVestingDuration.convertDurationToString("fully vested"),
