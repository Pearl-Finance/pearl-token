 ...
 * for CREATE2 address calculations. This salt ensures deterministic address generation for contracts deployed within
 * the Pearl ecosystem.
 * @dev The contract inherits DeploymentScriptBase's functionality, tailoring it to the Pearl ecosystem's deployment
 * needs. It uses the specific salt for all CREATE2 address calculations, providing consistency and predictability in
 * contract addresses.
 *
 * Key Features:
 * - Inherits the robust deployment and proxy management system of DeploymentScriptBase.
 * - Uses a specific salt to ensure deterministic and consistent CREATE2 address generation.
...
abstract contract PearlDeploymentScript is DeploymentScriptBase {
    constructor() DeploymentScriptBase("pearl.deployment-dev-2") {}
}
